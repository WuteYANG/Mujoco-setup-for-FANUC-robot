...
def IK(vec):
    ...
    """ All joint is 0 deg """
    if (px, py, pz, rx, ry, rz) == (0.55, 0.0, 0.475, math.pi, -math.pi/2, 0):
        q = np.zeros(6)
    ...
    # DH-parameters
    d = np.array([0.0000, 0.0000, 0.0000, -0.4200, 0.0000, -0.0800]) 
    ...
    beta2 = acos((l2*l2+a[1]*a[1]-l1*l1)/(2*l2*a[1]))
    ...
    if t5 > 0-0.00001 and t5 < 0+0.00001:
        t5 = 0
    elif rot[2,2] <= 0-0.00001:
    ...
    for i in range(3):
        for j in range(3):
            if rot[i,j] <= 0+0.0001 and rot[i,j] >= 0-0.0001:
                rot[i,j] = 0.0
            if R30[i,j] <= 0+0.0001 and R30[i,j] >= 0-0.0001:
                R30[i,j] = 0.0
    rott[:,1] = -rot[:,1]
    rott[:,2] = -rot[:,2]
    #print('rot', rot)
    ...
    if R36[2,0] == 0:
        ...
        t6 = atan2(-R36[2,1]/math.sin(t5), R36[2,0]/math.sin(t5))
    #print('theta4:', t4*180/math.pi)
    #print('theta6:', t6*180/math.pi)
    """
    Reorganize q
    """
    ## Singular point
